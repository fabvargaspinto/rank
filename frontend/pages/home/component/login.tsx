import Button from '@/ui/button/button';
import styles from './login.module.css';
import Link from 'next/link';
import Input from '@/ui/input/input';

function GoogleIcon() {
    return (
        <svg width="18" height="18" viewBox="0 0 48 48" aria-hidden="true">
            <path fill="#FFC107" d="M43.6 20.5H42V20H24v8h11.3C33.7 32.7 29.3 36 24 36c-6.6 0-12-5.4-12-12s5.4-12 12-12c3.1 0 5.8 1.2 8 3.1l5.7-5.7C34.2 6.1 29.4 4 24 4 12.9 4 4 12.9 4 24s8.9 20 20 20 20-8.9 20-20c0-1.2-.1-2.3-.4-3.5z" />
            <path fill="#FF3D00" d="M6.3 14.7l6.6 4.8C14.7 16 19 12 24 12c3.1 0 5.8 1.2 8 3.1l5.7-5.7C34.2 6.1 29.4 4 24 4 16.3 4 9.6 8.3 6.3 14.7z" />
            <path fill="#4CAF50" d="M24 44c5.2 0 10-2 13.6-5.2l-6.3-5.3C29.2 35.1 26.7 36 24 36c-5.3 0-9.7-3.3-11.3-8l-6.5 5C9.5 39.6 16.2 44 24 44z" />
            <path fill="#1976D2" d="M43.6 20.5H42V20H24v8h11.3c-1.1 3.2-3.5 5.8-6.6 7.5l6.3 5.3C38.2 37.3 44 31.5 44 24c0-1.2-.1-2.3-.4-3.5z" />
        </svg>
    );
}

export default function LoginForm() {
    return (
        <section className={styles.login}>
        <div className={styles.loginHeader}>
        <h1 className={styles.loginTitle}>Sello <span className={styles.loginTitleHighlight}>Nomada</span></h1>
        <p className={styles.loginDescription}>Ingresa tus credenciales para acceder a tu cuenta</p>
            </div>

        <form className={styles.loginForm}>
            <Input  type="email" placeholder="Email" name="email" id="email" />
            <Input  type="password" placeholder="Password" name="password" id="password" />
            <div className={styles.loginFeedback}>
            <p className={styles.loginFeedbackText}></p>
            </div>
            <Button variant="primary" size="full">Iniciar sesión</Button>
        </form>
        <div className={styles.loginDivider}>
            <span>o</span>
        </div>

        <div className={styles.loginFormFooter}>
        <Button variant="secondary" size="full">
            <GoogleIcon />
            Iniciar con Google
        </Button>
        <p className={styles.loginFormFooterText}>
            ¿No tienes una cuenta? 
            <Link href="/register" className={styles.loginFormFooterTextLink}>
            &nbsp; Regístrate
            </Link>
        </p>
        </div>
        </section>
    );
}